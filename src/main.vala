...
namespace LAIR{
	class Lair : Scribe {
		...
		public Lair(string[] lspt, string[] scrpt, int screenW, int screenH, int verbosity){
            base(verbosity);
            if (!help){
                ...
                if (SDLImage.init(imgInitFlags) != 0) {
                    //log some more shit.
                }
                SDLTTF.init();
                GameMap = new Game(lspt, scrpt, screenW, screenH);
            }else{
                message("***********************************************************************************");
                message("<I      <I I>         <I I>      I>");
                message(" |       | |           | |       |");
		...
		public static int main(string args[]){
			...
			var app = new Lair(listPaths, scriptPaths, PixelW, PixelH, Verbosity);
            if (!help){
                app.GameMap.run();
            }
            return app.goodbye();
